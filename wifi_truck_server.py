...
class WifiTruckServer:
	...
	def handleIncomingMsgs(self):
		while not self.disconnect:
			...
			if (currentMsg == "CMD_UPP"):
				print("FORWARD!")
				self.wifiTruckController.move(1.0)
			elif (currentMsg == "CMD_DWN"):
				print("BACKWARD!")
				self.wifiTruckController.move(0.0)
			elif (currentMsg == "CMD_LFT"):
				print("TURN LEFT!")
				self.wifiTruckController.steer(1.0)
			elif (currentMsg == "CMD_RGT"):
				print("TURN RIGHT!")
				self.wifiTruckController.steer(0.0)
			elif (currentMsg == "CMD_RL1"):
				print("RELEASE SERVO 1")
				self.wifiTruckController.releaseSteering()
			elif (currentMsg == "CMD_RL2"):
                                print("RELEASE SERVO 2")
                                self.wifiTruckController.releaseMovement()
			elif (currentMsg == "DSCNNCT"):
				print("Disconnect!")
				self.disconnect = True
				self.wifiTruckController.stop()
			else:
				print("else - currentMsg:" + currentMsg)
			self.outgoingMsgQueue.put("Server received:" + currentMsg)
